...
/usr/bin/time /usr/bin/rclone copy -vv --exclude "/logs/" --dump headers --fast-list --no-traverse --max-age 70m --min-age 5m --checksum --log-file=/logdir/sync-log-hourly-${dt}.log --transfers 2 --s3-chunk-size 16M /mnt/borgdirectory remote:bucket-name > /logdir/sync-time-hourly-${dt}.out 2>&1
